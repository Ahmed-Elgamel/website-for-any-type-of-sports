...
GO;
CREATE FUNCTION viewAvailableStadiumsOn1 (@datetime DATETIME)   --CHECK AGAIN NOT SURE !!!!!
RETURNS @output_table TABLE
(
STADIUM_NAME VARCHAR(20),
STADIUM_LOCATION VARCHAR(20),
...
Insert into @output_table
  ...
  LEFT OUTER JOIN Match M
  ON S.id=M.host_stadium_id
  WHERE S.status='1' AND  (@datetime NOT BETWEEN M.start_time AND M.end_time)  OR (M.id IS NULL) AND 
  --THE START AND ENDING TIMES OF MATCH MUST BE CHECKED IF THEY WILL BE CHANGE TO ANOTHER DATATYPE     done
...
SELECT @match_id=M.id ,@stadium_id=host_stadium_id            --GET ID OF THE MATCH AND STADIUM BASED ON THESE CONDITIONS
FROM Match M
...
update Ticket                     --change ticket status to sold
set status = '0'  --0 means sold
where id=@ticket_id

/*
update Stadium                    --decrement the capcity of stadium
